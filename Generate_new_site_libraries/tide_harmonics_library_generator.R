...
libexample2 = 'Tide_calculator_check.ino' # generic name for example Arduino sketch
...
sink(file = paste(libdirname,'/examples/Tide_calculator_check/',libexample,sep = ''), 
     type = 'output', split = TRUE, append = FALSE)
cat('/* ', libexample, '\n')
cat(' Copyright (c) ')
cat(strftime(curr.date, format = '%Y'))
...
cat('  // Expect second next\n')
cat('  mysec = Serial.parseInt();\n')
cat('\n')  
cat('  // When the enter symbol newline comes along, convert the \n')
cat('  // values to a DateTime object and set the clock\n')
cat('  if (Serial.read() == '\n'){\n')  ### CHECK ME
cat('    myTime = DateTime(myyear,mymonth,myday,myhour,myminute,mysec);\n')
cat('  // Calculate new tide height based on current time\n')
